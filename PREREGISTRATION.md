...
|---|---|---|---|---|
| `argmax-prereg-margin-desc-v2.0` | 2026-08-14 | `a7806343297d` | MD3, MD4 **REGISTERED AND UNEVALUATED** | Second-model replication of the descriptive margin claims on `Qwen/Qwen3.5-9B` with the thinking phase disabled, 198 problems at M=16, cap 6144. Design, calibration, stratification check and limitations in `notes/margin_desc_v2.md`. Covers no gate claim, and explicitly does NOT cover MD1 or MD2, which are not testable at M=16. **Not evaluated.** Cap 6144 was chosen from a 32-sample probe measuring answer rate 1.0000; the run measured 0.8931 with truncation 0.2905, failing the registered comparability condition `md_v2_answer_rate_reference` = 0.9950. Config drift was ruled out: one param_hash, zero reasoning tokens. Evaluating on a pool selected for finishing fastest would violate doc 2 s7.1 and doc 4 s4.1. See `notes/v2_comparability_failure.md`. |
