...
% The style file sets \lhead inside the \vbox in \@maketitle, where the
% assignment is group-local and discarded. Set it here at top level instead.
% Wording differs from the camera-ready: this source also builds the arXiv
% version, where "Accepted at" reads correctly alongside arXiv's own stamp.
\lhead{Accepted at the COLM 2026 Workshop on Efficient Reasoning}
...
The mechanism it supports: \emph{the answer token is a near-deterministic
readout of a chain that has already committed}. A sample whose answer
...
alternative option. Dissenting samples are as committed as agreeing ones. The
variance self-consistency exploits therefore does not live at the answer token;
it lives upstream, in which chain got written. This is a mechanism rather than
...
the mean difference is $-0.0168$ $[-0.0527, +0.0182]$, which crosses zero. The
population differs because a paired difference is undefined where a problem has
...
\textbf{No margin gate was built.} We measured the signal and report that it
cannot support the routing decision. We did not construct a margin-thresholded
gate and evaluate its accuracy, which is why Section~\ref{sec:gates} remains a
two-gate result.
...
estimation of an optimal call count: on a uniformly hard benchmark, the
...
\textbf{The bill.} Probes across all phases cost \$0.1346 over 148 samples; the
margin run on the v1 model cost \$3.4123 over 12,672 samples; the incomplete
second-model run cost \$1.1207 over 1,253 samples. Realized total across all
phases is \textbf{\$4.6676} over 14,073 samples, and the rows are disjoint.
\citet{souza2026budget} report 56,476 API calls and no monetary cost. We report
the ledger because the negative results in this paper are only interpretable
alongside what was affordable. The bill is computed, not confirmed: every row
is stored token counts multiplied by a dated price snapshot, the ledger is
complete against the raw store (14,073 rows against 14,073 sample records) and
every row recomputes to \$4.667639 with no disagreements, and the ledger is
published in the open record so the arithmetic is checkable. The provider
documents no balance or usage endpoint, and a dashboard balance reading cannot
close the loop: a balance alone cannot distinguish an undercounting ledger from
an unrecorded starting figure. \textbf{A provider-issued total spend figure, to
compare against \$4.667639, is not available and is the one number in this
paper we could not obtain.}
...
and paid for, and the alternative was a single dropped footnote away.
